...
router.delete("/sign-out", AuthMiddleware, async (req, res) => {
  ...
  if (!id) {
    return res
      .status(400)
      .json({ success: false, message: "사용자가 찾을 수 없습니다." });
  }
  ...
  return res
    .status(200)
    .json({ success: true, message: "사용자가 성공적으로 삭제되었습니다." });
});
...
router.post("/log-out", AuthMiddleware, async (req, res) => {
  ...
  if (!token) {
    return res
      .status(400)
      .json({ success: false, message: "토큰이 존재하지 않습니다." });
  }
  ...
  return res
    .status(200)
    .json({ success: true, message: "로그아웃이 성공적으로 완료 되었습니다." });
});
